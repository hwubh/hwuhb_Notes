...
def part1_inverse_kinematics(meta_data, joint_positions, joint_orientations, target_pose):
    ...
    cycle_count = 0
    alpha = 16
    while cycle_count < 50 and np.linalg.norm(position_chain[-1]- target_pose) > 1e-2: #不知道取平方和计算会少吗？
        joint_angle = []
        ...
        jacobian = []
        for i in range(0, len(path)):
            current_position = position_chain[i]
            current_angle = joint_angle[i]
            r = current_position - position_chain[-1]
        ...
        orientation_chain[0] = rotation[0]
        for i in range(0, len(path) - 1):
            position_chain[i + 1] = position_chain[i] + orientation_chain[i].apply(offset_chain[i + 1])
            if i + 1 == len(path) - 1:
                orientation_chain[i + 1] = orientation_chain[i]
            else:
                orientation_chain[i + 1] = orientation_chain[i] * rotation[i + 1]
        
        for i in range(0, len(path) - 1):
            if i == 0:
                rotation_chain[0] = orientation_chain[0]
            else:
                rotation_chain[i] = orientation_chain[i].inv() * orientation_chain[i + 1]
